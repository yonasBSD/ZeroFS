...
The optional `storage_class` field under `[storage]` sets the object storage class/tier for every write. The value is passed through verbatim.
...
Because the value is verbatim, it must be valid for your backend. Omit the field to use the account/bucket default. The typical use is selecting a cheaper single-zone / reduced-redundancy **hot** or higher performance class where your provider offers one.
...
- **`zstd-{level}`** (default: `zstd-3`): Zstandard compression with configurable level from 1 to 22. Lower levels (1-5) are faster, higher levels (15-22) achieve better compression but are slower. Level 3 strikes a good balance for object-storage workloads.
